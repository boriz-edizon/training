function init() {
    ...
    canvas.addEventListener("wheel", function(e) {
        if(!horizontalScroll){
            yTravelled += ((e.deltaY/1000) * (getAtInt(trackY, "height") - getAtInt(sliderY, "height")))
            yScroll()
        }
    });
    // y scroll
    function yScroll() {
        if (yTravelled < 0) {
            ...
            redraw(1)
        } else if (yTravelled > 0.8 * (getAtInt(trackY, "height") - getAtInt(sliderY, "height")) ){
            ...
            yTravelled = 0.5 * (getAtInt(trackY, "height") - getAtInt(sliderY, "height"))  
        } else {

            sliderY.style.top = yTravelled + "px";
            sliderPercentageY = (yTravelled/  (getAtInt(trackY, "height") - getAtInt(sliderY, "height"))*100)
    ...
    function xTravel(e) {
        xTravelled += ((e.deltaY/1000) * (getAtInt(trackX, "width") - getAtInt(sliderX, "width")))
        xScroll()
    }
    ...
    function xScroll(){
        if (xTravelled < 0) {
            ...
            redraw()
        } else if (xTravelled > 0.8 * (getAtInt(trackX, "width") - getAtInt(sliderX, "width")) ){
            ...
            xTravelled = 0.5 * (getAtInt(trackX, "width") - getAtInt(sliderX, "width"))  

        } else {
            sliderX.style.left = xTravelled + "px";
            sliderPercentageX = (xTravelled /  (getAtInt(trackX, "width") - getAtInt(sliderX, "width"))*100)
